...
//import akka.stream.scaladsl.Source.{fromIterator, future, futureSource}
...
//
//class RetryPushNotificationsJobSpec
...
////    }
//    val mockLockKeeper: LockService = new LockService {
//      override val lockRepository: MongoLockRepository = mock[MongoLockRepository]
//      override val lockId: String = "testLock"
//      override val ttl: duration.Duration = duration.Duration.create(5, duration.MINUTES)
...
//      mockLockKeeper.lockRepository.asInstanceOf[MongoLockRepository],
...
//        .thenReturn(future(fromIterator(() => Seq(retryableNotification).toIterator)))
...
//      when(mockNotificationsRepository.fetchRetryableNotifications)
//        .thenReturn(futureSource[RetryableNotification, NotUsed](failed(new RuntimeException("Failed"))))
//
//      val result: underTest.Result = await(underTest.execute)
//
//      verify(mockNotificationPushService, never).handlePushNotification(*, *)(*, *)
